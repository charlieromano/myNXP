...
#if defined(gAppUseSensors_d) && (gAppUseSensors_d > 0)
    /* Allows Sensors module to disable low power entry during an ADC measurement */
...
void APP_InitServices(void)
{
...
#endif
    /* Initialize Timers for Application, you can comment this out if no timer is used on your application */
    ...
    BOARD_InitBlueLed(gBoardLedBlueHdl);
#endif

#if (defined(gBoardLedGreen_d) && (gBoardLedGreen_d == 1))
    BOARD_InitGreenLed(gBoardLedGreenHdl);
#endif
