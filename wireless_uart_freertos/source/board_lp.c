...
/*!
 * \brief Wait application to be ready before going in lowpower
 *
 * \details Wait for all transmission over UART to complete before going in lowpower
 *
 */
static void BOARD_WaitAppReadyBeforeLowpower(void);
...
static void BOARD_EnterLowPowerCb(void)
{
    ...
    BOARD_WaitAppReadyBeforeLowpower();
...
#endif /* (BOARD_APP_UART_INSTANCE==1) */
...
#if !defined(gAppLpuart0WakeUpSourceEnable_d) || (gAppLpuart0WakeUpSourceEnable_d == 0) || (BOARD_APP_UART_INSTANCE == 0)
    BOARD_ReinitSerialManager2((serial_handle_t)gSerMgrIf2);
#endif /* defined(gAppUseSerialManager_c) && (gAppUseSerialManager_c > 1) */
#endif /* !defined(gAppLpuart0WakeUpSourceEnable_d) || (gAppLpuart0WakeUpSourceEnable_d==0) || (BOARD_APP_UART_INSTANCE==0)*/
